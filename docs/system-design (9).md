...
| --- | --- | --- | --- |
| `user_role` | `users.role` | `student`, `instructor` | `student` |
...
sequenceDiagram
    ...
    Note over TW,ES: on failure of the three steps above, compensation removes the enrollment row
...
- **Tracing.** OpenTelemetry instruments all five FastAPI services — API gateway, user
  service, course service, enrollment service, and analytics service. Trace context
  propagates over HTTP, so a request through the gateway appears as one connected trace
  across services in Jaeger. Two known boundaries: the Temporal worker is not
  OTel-instrumented, so workflow steps are observed in the Temporal UI rather than in
  Jaeger; and analytics' Kafka consumer runs on a background thread that
  `FastAPIInstrumentor` does not cover, so consumed events are not traced.
- **Metrics.** The gateway, course service, and enrollment service expose `/metrics`;
  Prometheus scrapes them; Grafana visualizes rates and latencies.
