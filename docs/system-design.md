...
flowchart TB
    ...
    subgraph async [Event and workflow backbone]
        TMP[Temporal]
...
The system has **two workflows**, and both are modeled as Temporal workflows
because both are ordered, multi-step processes that must not corrupt on partial
failure. Their independent *side effects* are then decoupled via Kafka.

- **Course publishing — orchestration (Temporal).** A defined, ordered, multi-step
  process (chunk content → generate embeddings → mark ready) that must not corrupt
  on partial failure → durable workflow with retries.
- **Enrollment — orchestration (Temporal saga) + choreography (Kafka).** The
  enrollment *transaction* is itself an ordered, must-all-succeed process that spans
  more than one service: validate the course, reserve a seat (Course Service),
  create the enrollment record, initialize progress. Because these steps cross a
  service boundary they can't be wrapped in one database transaction, so Temporal
  orchestrates them as a **saga** — if a later step fails, it runs compensations
  (release the reserved seat) so the system never ends up half-enrolled. Only when
  the workflow **commits** does it emit a single `StudentEnrolled` event; Kafka then
  fans that event out to analytics and notifications, which react **independently**.
  Temporal guarantees the transaction; Kafka decouples the side effects.
...
(Temporal); independent reactions after the fact → choreography (Kafka).** Both
workflows use orchestration for the core transaction; enrollment additionally uses
choreography for its downstream side effects.
...
sequenceDiagram
  ...
  participant DOC as NoSQL content store
  participant V as Vector DB
  I->>GW: POST /api/courses/{id}/publish
  GW->>CS: forward, JWT instructor
  CS->>T: start PublishWorkflow
  CS-->>GW: 202 Accepted, processing
  GW-->>I: 202 Accepted
  T->>W: activity, extract and chunk content
  ...
  Note over T,W: On failure Temporal retries the activity
  Note over T,W: Course stays not-ready until all steps succeed
```

### 10.4 Enrollment — Temporal saga, then Kafka fan-out **[PLANNED · W2/W3]**

Enrollment is the system's **second workflow**. The transaction spans two services
(Enrollment reserves a seat from Course), so it can't live in one database
transaction — Temporal orchestrates it as a **saga** with compensation. Only after
the workflow commits does it emit `StudentEnrolled`, which Kafka fans out to the
independent side-effect workers.
...
sequenceDiagram
  ...
  ES->>T: start EnrollmentWorkflow
  ES-->>GW: 202 Accepted, processing
  GW-->>S: 202 Accepted
  Note over T: Saga - ordered activities, compensate on failure
  T->>CS: 1. validate course published and has capacity
  CS-->>T: ok
  T->>CS: 2. reserve a seat
  CS-->>T: reserved
  T->>ES: 3. create enrollment record
  ES->>EDB: insert enrollment, unique student and course
  alt a step fails after the seat was reserved
    T->>CS: compensate - release seat
    T->>ES: mark enrollment FAILED
  else all steps succeed
    T->>ES: 4. initialize progress
    ES->>EDB: insert progress rows
    T->>K: 5. publish StudentEnrolled event
  end
  par analytics
    K->>AW: StudentEnrolled
    AW->>AW: update enrollment and popularity metrics
  and notifications
    K->>NW: StudentEnrolled
    NW->>NW: send welcome, dedup by event key
  end
  Note over T,EDB: Temporal guarantees the transaction with retries and compensation
  Note over T,EDB: Kafka fans out side effects only after it commits
```

Why a saga and not a single DB write: reserving a seat mutates **courses_db**
(Course Service) while the enrollment row lives in **enrollment_db** (Enrollment
Service). Two databases, two services — no shared transaction is possible, so the
"do all of these or undo what you did" guarantee has to come from the workflow
engine. That is exactly the gap a Temporal saga fills, and it is why enrollment is
a workflow rather than a plain endpoint.
...
- **Reliability & failure handling.** Temporal makes both multi-step workflows
  recoverable — publishing retries its activities, and the enrollment saga
  compensates (releases a reserved seat) if a later step fails, so an enrollment is
  never left half-committed across the two services. Kafka + Celery make
  side-effects retryable and absorb spikes (backpressure); partial failures never
  corrupt the primary record; failures surface as the "Failed Events" metric.
...
| Temporal                | Durable orchestration of both workflows — publishing and the cross-service enrollment saga (with compensation) |
...
| Workflows/async   | —                     | Temporal: publishing + enrollment saga | Kafka + Celery fan-out | embedding pipeline| streaming Q&A   |
