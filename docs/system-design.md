...
One detailed view of the whole SmartCourse architecture. Arrows are labeled with
what triggers them, so the diagram shows not just the components but the flows and
the key design decisions between them. Part A is built; the AI layer (Part B) is
shown dashed as the planned GenAI extension on the same foundation.
...
flowchart TD
    ...
    US -->|owns| UDB[(users_db<br/>Postgres)]
    CS -->|owns| CDB[(courses_db<br/>Postgres)]
    ES -->|owns| EDB[(enrollment_db<br/>Postgres)]

    %% Decision: one database per service, no cross-service FKs

    %% Orchestration: Temporal for multi-step, recoverable jobs
    CS -->|start CoursePublishingWorkflow| TMP{{Temporal<br/>durable orchestration}}
    ES -->|start EnrollmentWorkflow| TMP
    TMP --> WK[Temporal Worker<br/>runs workflow steps]
    WK -->|internal HTTP, never DB<br/>validate, set status, get content| CS
    WK -->|internal HTTP, never DB<br/>record, init progress, emit| ES

    %% Events: everything that happened is announced on Kafka
    US ==>|UserRegistered| K[[Kafka event backbone]]
    CS ==>|CoursePublished| K
    ES ==>|StudentEnrolled| K
    ES ==>|CourseCompleted| K
    AI -.->|AIAssistantUsage| K

    %% Analytics consumes all events, idempotent by event_id
    K ==>|consume all topics| AN[Analytics Service]
    AN -->|counters, metrics| MDB[(analytics_db<br/>MongoDB)]

    %% Notifications: slow work offloaded to a background worker
    AN -->|enqueue welcome task| RMQ[[RabbitMQ<br/>task queue]]
    ...
    CORE[Gateway, Course, Enrollment] -.->|OpenTelemetry traces| JAEG[Jaeger]
    CORE -.->|/metrics scraped| PROM[Prometheus] --> GRAF[Grafana]
...
- **One entry point.** All client traffic goes through the API Gateway, which strips
  the `/api` prefix and forwards to the right service.
- **One database per service, no cross-service foreign keys.** Each core service owns
  its own Postgres database; cross-service references are soft UUIDs kept consistent
  by tokens and events.
- **Temporal orchestrates multi-step, must-not-corrupt jobs.** Publishing and
  enrollment each start a durable Temporal workflow. The worker runs the steps with
  retries and, on failure, compensation.
- **The worker never touches a database.** Its activities call the owning service's
  internal HTTP endpoints (labeled on the worker arrows), so each service stays the
  sole owner of its data.
- **Everything that happened is announced as a Kafka event** (double arrows). This
  decouples the core flows from their reactions and lets new consumers be added
  without changing the producers.
- **Analytics consumes every event and is idempotent by event id**, so redelivered
  events never double-count; metrics land in MongoDB.
- **Slow work is offloaded.** The welcome notification is queued to RabbitMQ and run
  by a Celery worker (results in Redis, email caught by Mailpit), so it never blocks
  the request.
- **Observability is built in.** The three core services emit OpenTelemetry traces to
  Jaeger and expose metrics to Prometheus and Grafana.
- **The AI layer (dashed) is planned** on the same foundation: publishing feeds an
  indexing pipeline into a vector database, and the AI service answers questions using
  retrieval plus an LLM, emitting usage events back into the same analytics pipeline.
