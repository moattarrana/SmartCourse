...
One view of the whole SmartCourse architecture. Part A (core services, workflows,
events, analytics, observability) is built. Part B (the GenAI learning assistant —
AI service, embeddings, vector DB, LLM) is shown as part of the same target
architecture and is planned. Everything shares one service and event foundation.
...
flowchart TD
    ...
    CS -- on publish, index content --> IDX[Indexing Pipeline - Part B]
    IDX --> EMB[Embeddings - Part B]
    EMB --> VDB[(Vector DB - Part B)]
    AI --> EMB
    AI --> VDB
    AI --> LLM[LLM Provider - Part B]
...
endpoints. Analytics and notifications happen off the request path via Kafka events
and a Celery worker. The Part B GenAI layer (AI service, embeddings, vector DB, LLM)
reuses the same foundation: content is indexed when a course is published, and the AI
service answers questions and generates content using retrieval over the vector DB
plus the LLM, emitting usage events back into the same analytics pipeline.
