...
One detailed view of the whole SmartCourse architecture, including the internal
layering of the Temporal worker and the services. Arrows are labeled with what
triggers them. Part A is built; the AI layer (Part B) is shown dashed as the
planned GenAI extension on the same foundation.
...
flowchart TD
    ...
    subgraph CScomp[Course Service internals]
        CSpub[Public API routes] --> CSsvc[Service layer]
        CSint[Internal API<br/>X-Internal-Key] --> CSsvc
        CSsvc --> CSmod[Models] --> CDB[(courses_db<br/>Postgres)]
        CSsvc -->|CoursePublished| PROD1[Event producer]
    end
    CS --> CSpub

    subgraph EScomp[Enrollment Service internals]
        ESpub[Public API routes] --> ESsvc[Service layer<br/>enrollment + progress]
        ESint[Internal API<br/>X-Internal-Key] --> ESsvc
        ESsvc --> ESmod[Models] --> EDB[(enrollment_db<br/>Postgres)]
        ESsvc -->|StudentEnrolled, CourseCompleted| PROD2[Event producer]
    end
    ES --> ESpub

    US --> USsvc[Service layer] --> UDB[(users_db<br/>Postgres)]
    USsvc -->|UserRegistered| PROD3[Event producer]

    CSpub -->|start CoursePublishingWorkflow| TMP{{Temporal server<br/>durable state}}
    ESpub -->|start EnrollmentWorkflow| TMP

    subgraph WORKER[Temporal Worker - no database connection]
        direction TB
        WF1[Publishing Workflow<br/>order, retries, compensation]
        ACT1[Activities<br/>validate, process, mark_published, rollback]
        CC[CourseServiceClient<br/>HTTP + X-Internal-Key]
        WF2[Enrollment Workflow<br/>order, retries, compensation]
        ACT2[Activities<br/>record, init_progress, emit, rollback]
        EC[EnrollmentServiceClient<br/>HTTP + X-Internal-Key]
        WF1 --> ACT1 --> CC
        WF2 --> ACT2 --> EC
    end
    TMP -->|course-publishing queue| WF1
    TMP -->|enrollment-processing queue| WF2
    CC -->|internal HTTP, never DB| CSint
    EC -->|internal HTTP, never DB| ESint

    PROD1 ==> K[[Kafka event backbone<br/>events carry event_id]]
    PROD2 ==> K
    PROD3 ==> K
    AI -.->|AIAssistantUsage| K

    subgraph ANcomp[Analytics Service internals]
        CONS[Kafka consumer<br/>all topics] --> LEDG[Idempotency ledger<br/>dedupe by event_id]
        LEDG --> MET[Metrics updater] --> MDB[(analytics_db<br/>MongoDB)]
        API2[analytics read API] --> MDB
        CONS -->|enqueue welcome task<br/>enrollment only| RMQ[[RabbitMQ<br/>task queue]]
    end
    K ==>|consume all topics| CONS

    RMQ --> CW[Celery Worker]
    CW -->|SMTP| MP[Mailpit inbox]
    CW -->|task result| RED[(Redis<br/>result backend)]

    CSsvc -.->|on publish: chunk content| IDX[Indexing Pipeline - Part B]
    ...
    GW -.->|OpenTelemetry| JAEG[Jaeger]
    CS -.->|OpenTelemetry| JAEG
    ES -.->|OpenTelemetry| JAEG
...
How to read the diagram, layer by layer:

- **One entry point.** All client traffic goes through the API Gateway.
- **Each core service is layered.** A public API (routes) and a separate internal API
  (guarded by `X-Internal-Key`) both call the same service layer, which uses models to
  read and write the service's own database. The service layer also runs an event
  producer. This is the same shape in every service.
- **One database per service, no cross-service foreign keys.** Cross-service
  references are soft UUIDs kept consistent by tokens and events.
- **Temporal orchestrates in three layers, and the worker has no database.** For each
  of the two workflows: the Workflow orchestrates (order, retries, compensation); it
  calls thin Activities; the Activities call a Client that owns the HTTP and the
  internal key. The Client calls the owning service's internal API, and the service
  writes its own database. The worker never opens a database connection.
- **Two task queues.** The single worker process serves two Temporal queues,
  course-publishing and enrollment-processing, one per workflow.
- **Events decouple the reactions.** Each service produces events onto Kafka; every
  event carries an `event_id` used for idempotency.
- **Analytics is internally split.** A Kafka consumer reads all topics, an idempotency
  ledger dedupes by `event_id`, a metrics updater writes MongoDB, and a separate read
  API serves `/analytics`. The consumer also enqueues the welcome-email task, only for
  enrollment events.
- **Slow work is offloaded.** The welcome email is queued to RabbitMQ and run by a
  Celery worker (results in Redis, mail caught by Mailpit), off the request path.
- **Observability is built in.** The three core services emit OpenTelemetry traces to
  Jaeger and metrics to Prometheus and Grafana.
...
The endpoint does fast pre-checks, then starts the workflow, which durably records
the enrollment, initializes progress, and emits the event. The worker never touches
the database; it calls enrollment-service's internal endpoints. Kafka and Celery
handle the downstream fan-out unchanged.
...
### 7.3 Event fan-out — analytics and notifications
