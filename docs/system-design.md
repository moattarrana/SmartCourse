...
sequenceDiagram
  ...
  ES->>T: start EnrollmentWorkflow(student, course)
  ES-->>GW: 202 Accepted (status: processing)
  GW-->>S: 202 Accepted
  Note over T: Saga — ordered activities, compensate on failure
  T->>CS: 1. validate course published & has capacity
  CS-->>T: ok
  ...
  Note over T,EDB: Temporal guarantees the transaction (retries + compensation);<br/>Kafka fans out the side effects only after it commits
```
